...
knb_files <- list.files(here("knb-lter-cap_256_9"), 
                        pattern = "*.csv")
...
```{r}
bird_counts <- ggplot(data = selected_parks,
       aes(x = common_name,
       y = total_count)) +
  geom_col() +
...
Write a for loop, where, starting with the vector, you add a value and the one that immediately precedes it, returning a new vector with those sums.

```{r}
library(tidyverse)

vec1 <- c(2, 4, 6, 8, 10, 12)
sums <- vector(length = 5L)

# this will veturn a vector 
for (i in 2:length(vec1)) {
  print(vec1[i] + vec1[i - 1])
}

for (i in 2:length(vec1)) {
  sums_values <- vec1[i] + vec1[i - 1]
  sums[i - 1] <- sums_values
}
```
